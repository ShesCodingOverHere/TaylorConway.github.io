---
layout: single
title: "NUMT Circos Visualization"
permalink: /code/numt_circos/
author_profile: true
---

# NUMT Circos Visualization

This script generates a circular genome visualization of nuclear mitochondrial DNA segments (NUMTs), linking mitochondrial genome regions to their corresponding locations in the nuclear genome.

The plot integrates:
- mtDNA gene annotation  
- NUMT fragment positions  
- genomic locations across chromosomes  
- link-based visualization of insertion events  

This provides a global view of NUMT distribution and origin.
...
# =========================
# Input data (generic paths)
# =========================
mt_hits <- read.table("data/input/mt_alignments.tsv", header = FALSE)
colnames(mt_hits) <- c("numt_id","flag","mt_chr","mt_start","cigar")

nuclear_headers <- readLines("data/input/nuclear_numts.fa")
nuclear_headers <- nuclear_headers[grepl("^>", nuclear_headers)]

mt_genes <- read.table("data/input/mt_annotation.bed", header = FALSE)
...
get_ref_length <- function(cigar) {
  ...
  sum(as.numeric(str_extract(ops, "[0-9]+")))
}
...
# =========================
nuclear <- tibble(header = nuclear_headers) %>%
  mutate(
...
for(i in 1:nrow(mt_genes)) {
  ...
  col_use <- gene_colors[ mt_genes$type[i] ]
...
- NUMTs are visualized as links between mitochondrial and nuclear genomes  
- mtDNA genes are color-coded by functional category  
- NUMT fragments are stacked to avoid overlap  
- Chromosomes are simplified for clarity and readability  
- Designed for exploratory analysis and publication figures  

---

## Key Insight

This visualization highlights the genomic distribution and mitochondrial origin of NUMTs, making it possible to identify insertion hotspots, clustering patterns, and chromosome-specific enrichment.
